...
sssdGamma <- function(conviction, power, equivalence, gamma_alpha.1, gamma_beta.1, gamma_alpha.2, gamma_beta.2,  
  ...
  getNormal <- function(u, targetP, params, delta_star, n_val, hyper){
    ...
    x_high <- modes[1] + qnorm(0.75)*sqrt(mat2[1,1]*(1-mat2[1,2]^2/(mat2[1,1]*mat2[2,2]))/n_val)
    x_low <- modes[1] - qnorm(0.75)*sqrt(mat2[1,1]*(1-mat2[1,2]^2/(mat2[1,1]*mat2[2,2]))/n_val)
    ...
    sigma1_raw <- optim(sqrt(mat2[1,1]*(1-mat2[1,2]^2/(mat2[1,1]*mat2[2,2]))/n_val), 
                        find_var_cond, x = pts_mode, pt1 = pts1, pt2 = pts2, 
                        control = list(warn.1d.NelderMead = FALSE))$par
    
    ## find the raw variance for the beta parameter
    xx_high <- modes[2] + qnorm(0.75)*sqrt(mat2[2,2]*(1-mat2[1,2]^2/(mat2[1,1]*mat2[2,2]))/n_val)
    xx_low <- modes[2] - qnorm(0.75)*sqrt(mat2[2,2]*(1-mat2[1,2]^2/(mat2[1,1]*mat2[2,2]))/n_val)
    ...
    pts_mode <- c(modes[2], -1*loglik(modes, y = yy_star2, mu = hyper[3,1], tau = hyper[3,2],
                                      kappa = hyper[4,1], lambda = hyper[4,2]))
    sigma2_raw <- optim(sqrt(mat2[2,2]*(1-mat2[1,2]^2/(mat2[1,1]*mat2[2,2]))/n_val), 
                        find_var_cond, x = pts_mode, pt1 = pts1, pt2 = pts2, 
                        control = list(warn.1d.NelderMead = FALSE))$par
...
## SSSD estimation for the informative prior settings; save output to .csv file
tic <- Sys.time()
for (k in 1:3){
  res_1 <- NULL
  res_2 <- NULL
  for (j in 1:100){
    print(j)
    res_temp <- sssdGamma(conviction = convictions[k], power = powers[k], equivalence = equivalences[k], gamma_alpha.1 = 2.11, gamma_beta.1 = 0.69,
    ...
    write.csv(res_1, paste0("ba_samps_pwr_2", k, ".csv"), row.names = FALSE)
    write.csv(res_2, paste0("ba_results_rest_2", k, ".csv"), row.names = FALSE)
  }
}
toc <- Sys.time()
toc - tic

## SSSD estimation for the uninformative prior settings; save output to .csv file
tic <- Sys.time()
for (k in 1:3){
  res_1 <- NULL
  res_2 <- NULL
  for (j in 1:100){
    ...
    write.csv(res_1, paste0("ba_samps_pwr_1", k, ".csv"), row.names = FALSE)
    write.csv(res_2, paste0("ba_results_rest_1", k, ".csv"), row.names = FALSE)
  }
}
toc <- Sys.time()
toc - tic

## get the appropriate means for Table 1
for (i in 1:2){
  for (j in 1:3){
    dat <- read.csv(paste0("ba_results_rest_",i,j,".csv"))
