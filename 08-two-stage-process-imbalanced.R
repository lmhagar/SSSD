...
## via the probability of agreement with imbalanced sample sizes (n_2 = qn_1 for q > 0)
sssdGammaq <- function(conviction, power, equivalence, gamma_alpha.1, gamma_beta.1, gamma_alpha.2, gamma_beta.2, q = 1,
  ...
  ## q: constant for imbalanced sample size determination (n_2 = qn_1 for q > 0)
  ...
  avar2 <- t(c(d_alpha2, d_beta2))%*%solve(Fish2)%*%c(d_alpha2, d_beta2)/q
  ...
  targetPower <- function(u, targetP, params, delta_star, n_val, q){
    ...
    realP <- pnorm(log(1 + delta_star), log(theta1/theta2), sqrt(Fish_ratio_mu)/sqrt(n_val)) - pnorm(log(1/(1 + delta_star)), log(theta1/theta2), sqrt(Fish_ratio_mu)/sqrt(n_val))
  ...
  for (i in 1:nrow(sob_pwr)){
    endpoints1_vec[i] <- targetPower(targetP = 0, n_val = ceiling(0.5*mu_start),
                                     ...
                                     delta_star = equivalence, u = sob_pwr[i,], q=q)
  ...
  Fisher_ratio <- function(alpha1, beta1, alpha2, beta2, kappa, q){
  ...
  Fisher_alpha1 <- function(alpha1){
    Fisher_ratio(alpha1, beta1 = beta1, alpha2 = alpha2, beta2 = beta2, kappa = threshold, q = q)
  }
  
  Fisher_beta1 <- function(beta1){
    Fisher_ratio(alpha1 = alpha1, beta1, alpha2 = alpha2, beta2 = beta2, kappa = threshold, q = q)
  }
  
  Fisher_alpha2 <- function(alpha2){
    Fisher_ratio(alpha1 = alpha1, beta1 = beta1, alpha2, beta2 = beta2, kappa = threshold, q = q)
  }
  
  Fisher_beta2 <- function(beta2){
    Fisher_ratio(alpha1 = alpha1, beta1 = beta1, alpha2 = alpha2, beta2, kappa = threshold, q = q)
  }
  ...
  avar_dd2 <- t(c(dd_alpha2, dd_beta2))%*%solve(Fish2)%*%c(dd_alpha2, dd_beta2)/q
  ...
  ## q is again constant for imbalanced sample size determination
  getNormal <- function(u, targetP, params, delta_star, n_val, hyper, q){
    ...
    ## generate a non-random sample from each group (different sample sizes)
    N2 <- round(q*ceiling(as.numeric(n_val)))
    LHS <- seq(0.5, ceiling(as.numeric(n_val)) - 0.5)/ceiling(as.numeric(n_val))
    y_star1 <- qgamma(LHS, gamma_alpha.1, gamma_beta.1)
    LHS <- seq(0.5, ceiling(as.numeric(N2)) - 0.5)/ceiling(as.numeric(N2))
  ...
  normp1 <- getNormal(u = rep(0.5,4), targetP = conviction, delta_star = equivalence, hyper = hyper_mat,q= q,
  ...
  N2 <- round(q*ceiling(mu_v2))
  LHS <- seq(0.5, ceiling(as.numeric(mu_v2)) - 0.5)/ceiling(as.numeric(mu_v2))
  y_star1 <- qgamma(LHS, gamma_alpha.1, gamma_beta.1)
  LHS <- seq(0.5, ceiling(as.numeric(N2)) - 0.5)/ceiling(as.numeric(N2))
  y_star2 <- qgamma(LHS, gamma_alpha.2, gamma_beta.2)
  
  normp2 <- getNormal(u = rep(0.5,4), targetP = conviction, delta_star = equivalence, hyper = hyper_mat, q= q,
  ...
  mat2 <- matrix(c(1/gamma_alpha.2, 1/gamma_alpha.2, 1/gamma_alpha.2, 
                   trigamma(gamma_alpha.2))/(trigamma(gamma_alpha.2)*gamma_alpha.2 - 1), nrow = 2)
  a2 <- qnorm(sob[,3], log(gamma_alpha.2), sqrt(mat2[1,1]/ceiling(N2)))
  b2 <- qnorm(sob[,4], log(gamma_beta.2) + rho2*(a2 - log(gamma_alpha.2))*(sqrt(mat2[2,2])/sqrt(mat2[1,1])), sqrt(1- rho2^2)*sqrt(mat2[2,2]/ceiling(N2)))
  ...
  var_results <- NULL
  for (i in 1:rep_var){
    var_temp <- getNormal(u = rep(0.5,4), targetP = conviction, delta_star = equivalence, hyper = hyper_mat,q= q,
    ...
    var_results <- rbind(var_results, c(var_temp, 2*qnorm(1 - alpha/2)*var_temp[2], 
                                        pnorm(log(1 + equivalence), var_temp[1], var_temp[2]) - pnorm(log(1/(1 + equivalence)), var_temp[1], var_temp[2])))
  }
  ...
  normplow <- getNormal(u = rep(0.5,4), targetP = conviction, delta_star = equivalence, hyper = hyper_mat,q= q,
                        params = c(gamma_alpha.1, gamma_beta.1, gamma_alpha.2, gamma_beta.2, threshold), n_val = ceiling(as.numeric(mu_low)))
  normppen <- getNormal(u = rep(0.5,4), targetP = conviction, delta_star = equivalence, hyper = hyper_mat,q= q,
                        params = c(gamma_alpha.1, gamma_beta.1, gamma_alpha.2, gamma_beta.2, threshold), n_val = ceiling(as.numeric(mu_pen)))
  normphigh <- getNormal(u = rep(0.5,4), targetP = conviction, delta_star = equivalence, hyper = hyper_mat,q= q,
...
for (k in 2){
  ...
  for (j in 1:100){
    print(j)
    res_temp <- sssdGammaq(conviction = convictions[k], power = powers[k], equivalence = equivalences[k], gamma_alpha.1 = 2.43, gamma_beta.1 = 0.79,
                          gamma_alpha.2 = 2.11, gamma_beta.2 = 0.69, threshold = 4.29, mu0.1 = informs[1,1], tau0.1 = informs[1,2],
                          kappa0.1 = informs[2,1], lambda0.1 = informs[2,2], mu0.2 = informs[3,1], tau0.2 = informs[3,2],
                          kappa0.2 = informs[4,1], lambda0.2 = informs[4,2], rep_pwr = 1024, rep_var = 256, q = 2)
    
    res_1 <- rbind(res_1, res_temp[[1]])
    res_2 <- rbind(res_2, res_temp[[2]])
    
    
    write.csv(res_1, paste0("ba_imb_samps_pwr_2", k, ".csv"), row.names = FALSE)
    write.csv(res_2, paste0("ba_imb_results_rest_2", k, ".csv"), row.names = FALSE)
  }
}
toc <- Sys.time()
toc - tic
...
for (k in 2){
  ...
  for (j in 1:100){
    print(j)
    res_temp <- sssdGammaq(conviction = convictions[k], power = powers[k], equivalence = equivalences[k], gamma_alpha.1 = 2.11, gamma_beta.1 = 0.69,
                          gamma_alpha.2 = 2.43, gamma_beta.2 = 0.79, threshold = 4.29, mu0.1 = 2, tau0.1 = 0.1,
                          kappa0.1 = 2, lambda0.1 = 0.1, mu0.2 = 2, tau0.2 = 0.1, kappa0.2 = 2, lambda0.2 = 0.1, rep_pwr = 1024, rep_var = 256, q = 2)
    
    res_1 <- rbind(res_1, res_temp[[1]])
    res_2 <- rbind(res_2, res_temp[[2]])
    
    write.csv(res_1, paste0("ba_imb_samps_pwr_1", k, ".csv"), row.names = FALSE)
    write.csv(res_2, paste0("ba_imb_results_rest_1", k, ".csv"), row.names = FALSE)
  }
}
toc <- Sys.time()
toc - tic

## get the appropriate means for Table E3
for (i in 1:2){
  for (j in 2){
    dat <- read.csv(paste0("ba_imb_results_rest_",i,j,".csv"))
